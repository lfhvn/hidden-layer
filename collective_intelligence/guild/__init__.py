"""Python access to the Guild collective-intelligence research package.

The "real" code lives in ``collective-intelligence/guild/guild/`` (dash-named
project directory, per the repo's dual-directory convention). This wrapper
loads it under the importable ``collective_intelligence.guild`` namespace.
"""
...
_base_dir = Path(__file__).resolve().parents[2] / "collective-intelligence" / "guild"
_code_dir = _base_dir / "guild"

_code_module = _load_package(__name__ + ".code", _code_dir, aliases=("guild",))
...
# Register key modules under both namespaces
for module_name in ["kernel", "metrics", "experiment"]:
    ...
    if module_path.exists():
        ...
        if spec and spec.loader:
            ...
            sys.modules[f"guild.{module_name}"] = module
            spec.loader.exec_module(module)
